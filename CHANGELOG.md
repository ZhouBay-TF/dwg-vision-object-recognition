...
- Ark 视觉默认切换为官方推荐的 Files API + Responses API 本地文件链路：以 `purpose=user_data` 上传本地准备图并用 `input_image.file_id` 分析；保留 `ARK_VISION_TRANSPORT=chat_base64` 作为兼容回退，并增加本地文件上传单元测试。
